...
class Places {
    static search(lat, lng, radius, type, keyword, price, setOptions, setImages, map) {
    ...
    static async searchByAddress(address, radius, type, keyword, price, setOptions, setImages, map) {
        ...
        fetch(url).then(response => {
            ...
            if(response.status == "OK") this.search(response.results[0].geometry.location.lat, response.results[0].geometry.location.lng, radius, type, keyword, price, setOptions, setImages, map);
            else alert("Search failed or returned zero results");
        });
    }
